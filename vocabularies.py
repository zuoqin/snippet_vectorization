...
class CodeVectorizeWordFreqDicts(NamedTuple):
    token_to_count: WordFreqDictType
    track_to_count: WordFreqDictType
    target_to_count: WordFreqDictType


class CodeVectorizeVocabs:
    def __init__(self, config: Config):
        self.config = config
        self.token_vocab: Optional[Vocab] = None
        self.track_vocab: Optional[Vocab] = None
    ...
    def _load_or_create(self):
        ...
        if self.config.is_loading:
            vocabularies_load_path = self.config.get_vocabularies_track_from_model_path(self.config.MODEL_LOAD_PATH)
    ...
    def _load_from_path(self, vocabularies_load_path: str):
        ...
        with open(vocabularies_load_path, 'rb') as file:
            ...
            self.track_vocab = Vocab.load_from_file(
    ...
    def _create_from_word_freq_dict(self):
        word_freq_dict = self._load_label_freq_dict()
        ...
        self.track_vocab = Vocab.create_from_freq_dict(
            VocabType.Path, word_freq_dict.track_to_count, self.config.MAX_TRACK_VOCAB_SIZE,
            special_words=self._get_special_words_by_vocab_type(VocabType.Path))
        self.config.log('Created path vocab. size: %d' % self.track_vocab.size)
    ...
    def save(self, vocabularies_save_path: str):
        ...
        with open(vocabularies_save_path, 'wb') as file:
            ...
            self.track_vocab.save_to_file(file)
        self._already_saved_in_paths.add(vocabularies_save_path)

    def _load_label_freq_dict(self) -> CodeVectorizeWordFreqDicts:
        ...
        with open(self.config.word_freq_dict_path, 'rb') as file:
            ...
            track_to_count = pickle.load(file)
            target_to_count = pickle.load(file)
        self.config.log('Done loading word frequencies dictionaries.')
        return CodeVectorizeWordFreqDicts(
            token_to_count=token_to_count, track_to_count=track_to_count, target_to_count=target_to_count)
