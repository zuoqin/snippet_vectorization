...
public class FeatureExtractor {
	...
	public ArrayList<SnippetFeatures> extractFeatures(String code) throws ParseException, IOException {
		...
		ArrayList<SnippetFeatures> programs = generateFeatures(methods);
	...
	public ArrayList<SnippetFeatures> generateFeatures(ArrayList<MethodContent> methods) {
		ArrayList<SnippetFeatures> methodsFeatures = new ArrayList<>();
		for (MethodContent content : methods) {
			if (content.getLength() < m_CommandLineValues.MinCodeLength
				...
				continue;
			SnippetFeatures singleMethodFeatures = generateFeaturesForFunction(content);
	...
	private SnippetFeatures generateFeaturesForFunction(MethodContent methodContent) {
		ArrayList<Node> functionEnds = methodContent.getLeaves();
		SnippetFeatures snippetFeatures = new SnippetFeatures(methodContent.getName());

		for (int i = 0; i < functionEnds.size(); i++) {
			for (int j = i + 1; j < functionEnds.size(); j++) {
				String separator = Common.EmptyString;

				String path = generatePath(functionEnds.get(i), functionEnds.get(j), separator);
				if (path != Common.EmptyString) {
					Property source = functionEnds.get(i).getUserData(Common.PropertyKey);
					Property target = functionEnds.get(j).getUserData(Common.PropertyKey);
					snippetFeatures.addFeature(source, path, target);
				}
			}
		}
