...
class Config:
    ...
    def arguments_parser(cls) -> ArgumentParser:
        ...
        parser.add_argument('--export_body_vectors', action='store_true', required=False,
                            help="export snippet vectors for the given examples")
        parser.add_argument('--release', action='store_true',
                            help='if specified and loading a trained model, release the loaded model for a lower model '
                                 'size.')
        parser.add_argument('--predict', action='store_true',
                            help='execute the interactive forecast shell')
    ...
    def set_defaults(self):
        ...
        self.TOP_USED_CONSIDERED_DURING_PREDICTION = 10
        ...
        self.MAX_TRACK_VOCAB_SIZE = 911417
        self.DEFAULT_EMBEDDINGS_SIZE = 128
        self.TOKEN_EMBEDDINGS_SIZE = self.DEFAULT_EMBEDDINGS_SIZE
        self.TRACK_EMBEDDINGS_SIZE = self.DEFAULT_EMBEDDINGS_SIZE
        self.BODY_VECTOR_SIZE = self.context_vector_size
        self.TARGET_EMBEDDINGS_SIZE = self.BODY_VECTOR_SIZE
    ...
    def load_from_args(self):
        ...
        self.MODEL_LOAD_PATH = args.load_path if args.load_path is not None else 'models/java14_model/saved_model_iter8.release'
        self.TRAIN_DATA_TRACK_PREFIX = args.data_path
        self.TEST_DATA_PATH = args.test_path
        self.RELEASE = args.release
        self.EXPORT_BODY_VECTORS = args.export_body_vectors
    ...
    def __init__(self, set_defaults: bool = False, load_from_args: bool = False, verify: bool = False):
        ...
        self.TOP_USED_CONSIDERED_DURING_PREDICTION: int = 0
        ...
        self.MAX_TRACK_VOCAB_SIZE: int = 0
        self.DEFAULT_EMBEDDINGS_SIZE: int = 0
        self.TOKEN_EMBEDDINGS_SIZE: int = 0
        self.TRACK_EMBEDDINGS_SIZE: int = 0
        self.BODY_VECTOR_SIZE: int = 0
    ...
    def context_vector_size(self) -> int:
        return self.TRACK_EMBEDDINGS_SIZE + 2 * self.TOKEN_EMBEDDINGS_SIZE

    @property
    def is_training(self) -> bool:
        return bool(self.TRAIN_DATA_TRACK_PREFIX)
    ...
    def train_data_path(self) -> Optional[str]:
        if not self.is_training:
            return None
        return '{}.train.c2v'.format(self.TRAIN_DATA_TRACK_PREFIX)

    @property
    def word_freq_dict_path(self) -> Optional[str]:
        if not self.is_training:
            return None
        return '{}.dict.c2v'.format(self.TRAIN_DATA_TRACK_PREFIX)

    @classmethod
    def get_vocabularies_track_from_model_path(cls, model_file_path: str) -> str:
    ...
    def get_logger(self) -> logging.Logger:
        if self.__logger is None:
            self.__logger = logging.getLogger('snippet vectorize')
