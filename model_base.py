...
from vocabularies import CodeVectorizeVocabs, VocabType
from config import Config


class ModelEvalResults(NamedTuple):
    topk_acc: float
...
class ModelForecastResults(NamedTuple):
    original_name: str
    used_predicted_words: np.ndarray
    used_predicted_words_scores: np.ndarray
    notice_per_context: Dict[Tuple[str, str, str], float]
    code_vector: Optional[np.ndarray] = None


class CodeVectorizeModelBase(abc.ABC):
    def __init__(self, config: Config):
        ...
        self.vocabs = CodeVectorizeVocabs(config)
    ...
    def _log_creating_model(self):
        ...
        self.log('---------------------------------------------------------------------')
        self.log('---------------------------------------------------------------------')
        self.log('---------------------- Creating code vectorize model ----------------')
        self.log('---------------------------------------------------------------------')
        self.log('---------------------------------------------------------------------')
    ...
    def load_or_build(self):
        self.vocabs = CodeVectorizeVocabs(self.config)
        self._load_or_create_inner_model()
    ...
    def save(self, model_save_path=None):
        ...
        self.vocabs.save(self.config.get_vocabularies_track_from_model_path(model_save_path))
        self._save_inner_model(model_save_path)

    def _write_snippet_vectors(self, file, snippet_vectors):
        for vec in snippet_vectors:
            file.write(' '.join(map(str, vec)) + '\n')

    def _get_notice_weight_per_context(
            self, track_source_strings: Iterable[str], track_strings: Iterable[str], track_target_strings: Iterable[str],
            notice_weights: Iterable[float]) -> Dict[Tuple[str, str, str], float]:
        notice_weights = np.squeeze(notice_weights, axis=-1)  # (max_contexts, )
        notice_per_context: Dict[Tuple[str, str, str], float] = {}
        # shape of track_source_strings, track_strings, track_target_strings, notice_weights is (max_contexts, )

        # iterate over contexts
        for track_source, path, track_target, weight in \
                zip(track_source_strings, track_strings, track_target_strings, notice_weights):
            string_context_triplet = (common.binary_to_string(track_source),
                                      common.binary_to_string(path),
                                      common.binary_to_string(track_target))
            notice_per_context[string_context_triplet] = weight
        return notice_per_context
    ...
    def save_word2vec_format(self, dest_save_path: str, vocab_type: VocabType):
        ...
        vocab_embedding_matrix = self._get_vocab_embedding(vocab_type)
