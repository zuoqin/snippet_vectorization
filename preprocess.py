...
def save_dictionaries(dataset_name, word_to_count, track_to_count, target_to_count,
    ...
    with open(save_dict_file_path, 'wb') as file:
        ...
        pickle.dump(track_to_count, file)
...
def process_file(file_path, data_file_role, dataset_name, word_to_count, track_to_count, max_contexts):
    sum_total = 0
    sum_sampled = 0
    total = 0
    empty = 0
            ...
            for line in file:
                ...
                if len(contexts) > max_contexts:
                    ...
                    full_found_contexts = [c for i, c in enumerate(contexts)
                                           if context_full_found(context_parts[i], word_to_count, track_to_count)]
                    partial_found_contexts = [c for i, c in enumerate(contexts)
                                              if context_partial_found(context_parts[i], word_to_count, track_to_count)
...
def context_full_found(context_parts, word_to_count, track_to_count):
    return context_parts[0] in word_to_count \
           and context_parts[1] in track_to_count and context_parts[2] in word_to_count


def context_partial_found(context_parts, word_to_count, track_to_count):
    return context_parts[0] in word_to_count \
           or context_parts[1] in track_to_count or context_parts[2] in word_to_count
...
if __name__ == '__main__':
    ...
    parser.add_argument("-pvs", "--track_vocab_size", dest="track_vocab_size", default=911417,
    ...
    parser.add_argument("-ph", "--track_histogram", dest="track_histogram",
                        help="track_histogram file", metavar="FILE", required=True)
    ...
    track_histogram_path = args.track_histogram

    word_histogram_data = common.common.load_from_histogram(word_histogram_path, start_from=1,
                                                                  max_size=int(args.word_vocab_size),
                                                                  return_counts=True)
    _, _, _, word_to_count = word_histogram_data
    _, _, _, track_to_count = common.common.load_from_histogram(track_histogram_path, start_from=1,
                                                                     max_size=int(args.track_vocab_size),
                                                                     return_counts=True)
    _, _, _, target_to_count = common.common.load_from_histogram(args.target_histogram, start_from=1,
    ...
    for data_file_path, data_role in zip([test_data_path, val_data_path, train_data_path], ['test', 'val', 'train']):
        num_examples = process_file(file_path=data_file_path, data_file_role=data_role, dataset_name=args.output_name,
                                    word_to_count=word_to_count, track_to_count=track_to_count,
                                    max_contexts=int(args.max_contexts))
        if data_role == 'train':
            num_training_examples = num_examples

    save_dictionaries(dataset_name=args.output_name, word_to_count=word_to_count,
                      track_to_count=track_to_count, target_to_count=target_to_count,
