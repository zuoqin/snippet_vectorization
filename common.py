...
class common:
    ...
    @staticmethod
    def _load_from_histogram(path, min_count=0, start_from=0, return_counts=False):
    ...
    @staticmethod
    def load_from_histogram(path, min_count=0, start_from=0, max_size=None, return_counts=False):
        if max_size is not None:
            word_to_index, index_to_word, next_index, word_to_count = \
                common._load_from_histogram(path, min_count, start_from, return_counts=True)
        ...
        return common._load_from_histogram(path, min_count, start_from, return_counts)
    ...
    @staticmethod
    def parse_forecast_results(raw_forecast_results, unhash_dict, special_words, topk: int = 5) -> List['MethodPredictResults']:
        forecast_results = []
        for single_method_forecast in raw_forecast_results:
            current_method_forecast_results = MethodPredictResults(single_method_forecast.original_name)
            for i, predicted in enumerate(single_method_forecast.used_predicted_words):
                if predicted == special_words.OOV:
                    continue
                suggestion_subtokens = common.get_subtokens(predicted)
                current_method_forecast_results.append_forecast(
                    suggestion_subtokens, single_method_forecast.used_predicted_words_scores[i].item())
            topk_notice_per_context = [
                (key, single_method_forecast.notice_per_context[key])
                for key in sorted(single_method_forecast.notice_per_context,
                                  key=single_method_forecast.notice_per_context.get, reverse=True)
            ][:topk]
            for context, notice in topk_notice_per_context:
                token1, hashed_path, token2 = context
                if hashed_path in unhash_dict:
                    unhashed_path = unhash_dict[hashed_path]
                    current_method_forecast_results.append_notice_path(notice.item(), token1=token1,
                                                                            path=unhashed_path, token2=token2)
            forecast_results.append(current_method_forecast_results)
    ...
    @staticmethod
    def squeeze_single_batch(arrays):
    ...
    @staticmethod
    def get_first_match(special_words, original_name, top_predicted_words) -> Optional[Tuple[int, str]]:
...
class MethodPredictResults:
    def __init__(self, original_name):
        self.original_name = original_name
        self.forecasts = list()
        self.notice_paths = list()

    def append_forecast(self, name, probability):
        self.forecasts.append({'name': name, 'probability': probability})

    def append_notice_path(self, notice_score, token1, path, token2):
        self.notice_paths.append({'score': notice_score,
                                     'path': path,
                                     'token1': token1,
                                     'token2': token2})
